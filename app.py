...
document.getElementById("downloadDocx").addEventListener("click", async function () {
    if (!latestPolicy || typeof latestPolicy !== "object") {
        alert("Generate a policy before downloading.");
        return;
    }

        const source = latestPolicy.source_ip || latestPolicy.source_address;
    const dest = latestPolicy.destination_ip || latestPolicy.destination_address;

    if (!source || !dest) {
        alert("Missing required fields in generated policy.");
        return;
    }
    ...
    const mappedPolicy = {
    source_ip: normalizeToArray(latestPolicy.source_ip || latestPolicy.source_address),
    destination_ip: normalizeToArray(latestPolicy.destination_ip || latestPolicy.destination_address),
...
def export_to_doc():
    try:
        ...
        print("📄 DOCX export received policy:", policy)
